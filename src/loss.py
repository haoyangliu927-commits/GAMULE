...
def soft_hyperedge_mutex_matrix(
    partition_soft: torch.Tensor,
    neg_mask: torch.Tensor,
    eps: float = 1e-8,
) -> torch.Tensor:
    """Aggregate gene-level mutual exclusion into a soft hyperedge profile matrix."""
    p = partition_soft
    neg = neg_mask.to(device=p.device, dtype=p.dtype)
    module_mass = p.sum(dim=0)
    denom = (module_mass[:, None] * module_mass[None, :]).clamp_min(eps)
    mutex = p.T @ neg @ p
    mutex = mutex / denom
    eye = torch.eye(mutex.shape[0], dtype=torch.bool, device=mutex.device)
    return mutex.masked_fill(eye, 0.0)


def directed_hyperedge_inclusion_loss(
    partition_soft: torch.Tensor,
    neg_mask: torch.Tensor,
    directed_inclusion_mask: torch.Tensor,
    *,
    margin: float = 0.0,
    min_direction_weight: float = 0.0,
    detach_direction_weights: bool = True,
    eps: float = 1e-8,
) -> torch.Tensor:
    """
    Penalize hyperedge-level violations of parent -> child mutex-profile inclusion.

    For a directed relation parent H_a -> child H_b, the parent mutex profile should
    be included in the child mutex profile. The violation is H_a-c > H_b-c.
    """
    p = partition_soft
    directed = directed_inclusion_mask.to(device=p.device, dtype=p.dtype)
    mutex = soft_hyperedge_mutex_matrix(p, neg_mask, eps=eps)

    module_mass = p.sum(dim=0)
    denom = (module_mass[:, None] * module_mass[None, :]).clamp_min(eps)
    direction_weight = (p.T @ directed @ p) / denom

    num_hyperedges = direction_weight.shape[0]
    idx = torch.arange(num_hyperedges, device=p.device)
    pair_valid = idx[:, None] != idx[None, :]
    direction_weight = direction_weight.masked_fill(~pair_valid, 0.0)
    if min_direction_weight > 0.0:
        direction_weight = torch.where(
            direction_weight >= min_direction_weight,
            direction_weight,
            torch.zeros_like(direction_weight),
        )
    if detach_direction_weights:
        direction_weight = direction_weight.detach()

    violation = F.relu(mutex[:, None, :] - mutex[None, :, :] - float(margin)) ** 2
    profile_valid = (
        (idx[None, None, :] != idx[:, None, None])
        & (idx[None, None, :] != idx[None, :, None])
    )
    valid = pair_valid[:, :, None] & profile_valid
    weights = direction_weight[:, :, None] * valid.to(dtype=p.dtype)
    total_weight = weights.sum()
    if float(total_weight.detach().item()) <= eps:
        return torch.zeros((), device=p.device, dtype=p.dtype)
    return (weights * violation).sum() / total_weight.clamp_min(eps)
